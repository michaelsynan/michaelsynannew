...
export default defineEventHandler(async (event) => {
  ...
  const fullPrompt = `
    ...
    3. Provide my email address, hello@michaelsynan.com, at an appropriate point in the conversation. Avoid repeating it unless explicitly asked by the user.
  ...
  const response = await fetchChatbotResponse(fullPrompt);
  return response;
});
